...
const SANCTIONED_VESSEL_NAMES = ['SHADOW', 'DARK', 'GHOST', 'PHANTOM', 'ARCTIC']
...
function checkSanctioned(name: string): boolean {
  return SANCTIONED_VESSEL_NAMES.some(s => name.toUpperCase().includes(s))
}
...
export async function GET() {
  ...
  const vessels = PROCEDURAL_VESSELS.map(v => ({
    ...v,
    sanctioned: checkSanctioned(v.name),
